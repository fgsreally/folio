...
export default class Camera {
    ...
    setAngle() {
        ...
        // Debug
        if (this.debug) {
            this.debugFolder.addInput(this, "easing", {
                label: "easing",
                min: 0,
                max: 1,
            });
            this.debugFolder.addInput(this.angle.value, "x", {
                label: "invertDirectionX",
                min: -2,
                max: 2,
            });
            this.debugFolder.addInput(this.angle.value, "y", {
                label: "invertDirectionY",
                min: -2,
                max: 2,
            });
            this.debugFolder.addInput(this.angle.value, "z", {
                label: "invertDirectionZ",
                min: -2,
                max: 2,
            });
        }
        ...
        this.renderer.domElement.addEventListener("touchmove", (_event) => {
            if (_event.touches.length === 2) {
                ...
                this.zoom.targetValue =
                    this.zoom.touch.startValue - (ratio - 1);
                this.zoom.targetValue = Math.min(
                    Math.max(this.zoom.targetValue, 0),
                    1
                );
    ...
    setPan() {
        ...
        this.renderer.domElement.addEventListener("touchstart", (_event) => {
            if (_event.touches.length === 1) {
                this.pan.down(
                    _event.touches[0].clientX,
                    _event.touches[0].clientY
                );
            }
        });
