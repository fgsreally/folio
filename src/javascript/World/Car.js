...
export default class Car {
    ...
    setChassis() {
        ...
        this.chassis.object.position.copy(
    ...
    setAntena() {
        ...
        this.time.on("tick", () => {
            ...
            const pullBack = position
                .negate()
                .multiplyScalar(
                    position.length() * this.antena.pullBackStrength
                );
        ...
        if (this.debug) {
            ...
            folder.addInput(this.antena, "speedStrength", {
                label: "speedStrength",
                min: 0,
                max: 50,
            });
            this.debugFolder.addInput(this.antena, "damping", {
                label: "damping",
                min: 0,
                max: 0.1,
            });
            this.debugFolder.addInput(this.antena, "pullBackStrength", {
                label: "pullBackStrength",
                min: 0,
                max: 0.1,
            });
            ...
            // folder
            //     .add(this.antena, "pullBackStrength")
            //     .step(0.0001)
            //     .min(0)
            //     .max(0.1);
        }
    ...
    setBackLights() {
        ...
        this.backLightsReverse.material =
            this.materials.pures.items.yellow.clone();
        ...
        // Time tick
        this.time.on("tick", () => {
            this.backLightsBrake.material.opacity = this.physics.controls
                .actions.brake
                ? 1
                : 0.5;
            this.backLightsReverse.material.opacity = this.physics.controls
                .actions.down
                ? 1
                : 0.5;
        });
    ...
    setTransformControls() {
        ...
        if (this.debug) {
            ...
            folder
                .addInput(this.transformControls, "enabled", {
                    label: "enabled",
                    min: 0,
                    max: 3,
                })
    ...
    setShootingBall() {
        ...
        window.addEventListener("keydown", (_event) => {
            if (_event.key === "b") {
                ...
                let direction = carPosition.vsub(
                    bowlingBall.collision.body.position
                );
                direction.normalize();
                direction = direction.scale(10);
    ...
    setKlaxon() {
        ...
        window.addEventListener("keydown", (_event) => {
            ...
            if (_event.key === "h" && this.klaxon.can) {
                ...
                this.sounds.play(
                    Math.random() < 0.002 ? "carHorn2" : "carHorn1"
                );
            }
