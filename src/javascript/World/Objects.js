...
export default class Objects {
    ...
    getConvertedMesh(_children, _options = {}, shaderHook) {
        ...
        // if (isHook) {
        //     console.log(baseChildren.length);
        // }
        for (const _child of baseChildren) {
            ...
            if (_child instanceof THREE.Mesh) {
                ...
                typeof shaderHook === "function" &&
                    (mesh.material.onBeforeCompile = (shader) => {
                        shaderHook(shader, _child, mesh);
                        ...
                        //                                     `   if(vPosition.x > 0.0) {
                        //                                         gl_FragColor = vec4(1., 0., 0., 1.); } else {
                        //                                             gl_FragColor = vec4(uShadowColor, shadowAlpha);
                        //                                     }
                        //                                    `
                        //                                 );
    ...
    add(_options) {
        ...
        let baseMesh =
            _options.mode === "divided"
                ? _options.base
                : _options.base.children;
        if (_options.mode === "test") {
            console.log(_options.base.children);
        }
        // Container
        ...
        let collisionMesh;
        if (_options.mode === "divided") {
            collisionMesh = _options.collision;
        } else {
            collisionMesh = _options.collision.children;
        }

        // console.log(_options.mode === "divided");
        if (_options.collision && this.physics) {
            ...
            if (_options.mass > 0) {
                this.time.on("tick", () => {
                    object.container.position.copy(
                        object.collision.body.position
                    );
