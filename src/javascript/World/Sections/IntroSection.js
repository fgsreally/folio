...
export default class IntroSection {
    constructor(_options) {
        ...
        this.x = _options.x || 0;
        this.y = _options.y || 0;
        this.fly = _options.fly;
        this.steam = _options.steam;
        // Set up
        ...
        loader.load(
            "https://prod.spline.design/5zsSYmnnLmovB8IA/scene.splinecode",
            (splineScene) => {
                this.splineScene = splineScene;
                splineScene.scale.set(0.01, 0.01, 0.01);
             
                this.container.add(splineScene);
                let collision = this.resources.items.test.scene;

                let collisionMesh = this.physics.addObjectFromThree({
                    meshes: [...collision.children],
                    offset: new THREE.Vector3(0, 0, 3),
                    rotation: new THREE.Euler(0, 0, 0),
                    mass: 1.5,
                    sleep: false,
                });
                for (const _child of splineScene.children) {
                    _child.position.sub(collisionMesh.center);
                }
                this.time.on("tick", () => {
                    splineScene.position.copy(collisionMesh.body.position);
                    splineScene.quaternion.copy(collisionMesh.body.quaternion);
                });
            }
        );
        this.setStatic();
    ...
    setOtherInstructions() {
        ...
        this.otherInstructions.horn = this.objects.add({
            mode: "divided",
            base: [
                this.resources.items.hornCombine.scene.children[1],
                this.resources.items.hornCombine.scene.children[2],
            ],
            collision: [
                this.resources.items.hornCombine.scene.children[3],
                this.resources.items.hornCombine.scene.children[4],
                this.resources.items.hornCombine.scene.children[5],
                this.resources.items.hornCombine.scene.children[0],
            ],
