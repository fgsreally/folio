...
export default class Walls {
    ...
    add(_options) {
        ...
        switch (_options.shape.type) {
            ...
            case "brick":
                for (let i = 0; i < heightCount; i++) {
                    ...
                    if (
                        _options.shape.type === "brick" &&
                        lastLine &&
                        _options.shape.equilibrateLastLine
                    ) {
                        if (i % 2 === 0) {
                            widthCountTemp--;
                        } else {
                    ...
                    for (; j < widthCountTemp; j++) {
                        ...
                        offset.add(
                            shape.offsetWidth
                                .clone()
                                .multiplyScalar(
                                    j - (shape.widthCount - 1) * 0.5
                                )
                        );
                        offset.add(
                            shape.offsetHeight.clone().multiplyScalar(i)
                        );
                        offset.x +=
                            (Math.random() - 0.5) * shape.randomOffset.x;
                        offset.y +=
                            (Math.random() - 0.5) * shape.randomOffset.y;
                        offset.z +=
                            (Math.random() - 0.5) * shape.randomOffset.z;

                        if (_options.shape.type === "brick" && i % 2 === 0) {
                            offset.add(
                                shape.offsetWidth.clone().multiplyScalar(0.5)
                            );
                        }

                        const rotation = new THREE.Euler();
                        rotation.x +=
                            (Math.random() - 0.5) * shape.randomRotation.x;
                        rotation.y +=
                            (Math.random() - 0.5) * shape.randomRotation.y;
                        rotation.z +=
                            (Math.random() - 0.5) * shape.randomRotation.z;
            ...
            case "triangle":
                ...
                for (let i = 0; i < heightCount; i++) {
                    for (let j = 0; j < widthCount; j++) {
                        ...
                        offset.add(
                            shape.offsetWidth.clone().multiplyScalar(i * 0.5)
                        );
                        offset.add(
                            shape.offsetHeight.clone().multiplyScalar(i)
                        );
        ...
        for (const _coordinates of wall.coordinates) {
            ...
            objectOptions.offset = _options.object.offset
                .clone()
                .add(_coordinates.offset)
                .add(shape.position);
