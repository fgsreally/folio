...
export default class Areas {
    ...
    setMouse() {
        ...
        window.addEventListener("mousemove", (_event) => {
            this.mouse.coordinates.x =
                (_event.clientX / window.innerWidth) * 2 - 1;
            this.mouse.coordinates.y =
                -(_event.clientY / window.innerHeight) * 2 + 1;
        ...
        this.renderer.domElement.addEventListener("touchstart", (_event) => {
            this.mouse.coordinates.x =
                (_event.changedTouches[0].clientX / window.innerWidth) * 2 - 1;
            this.mouse.coordinates.y =
                -(_event.changedTouches[0].clientY / window.innerHeight) * 2 +
                1;
        ...
        this.time.on("tick", () => {
            ...
            if (this.mouse.needsUpdate) {
                ...
                const intersects =
                    this.mouse.raycaster.intersectObjects(objects);

                // Intersections found
                if (intersects.length) {
                    // Find the area
                    const area = this.items.find(
                        (_area) => _area.mouseMesh === intersects[0].object
                    );
                    ...
                    if (area !== this.mouse.currentArea) {
                        ...
                        if (this.mouse.currentArea !== null) {
                            ...
                            this.mouse.currentArea.testCar =
                                this.mouse.currentArea.initialTestCar;
                        }
